  ...
  describe '.find' do
    ...
    context 'if args is INT' do
      it 'data should be stored in cache after find' do
        expect(ActiveCache.exist?("active_cache/user/find/#{user.id}")).to eq false
        User.find(user.id)
        expect(ActiveCache.exist?("active_cache/user/find/#{user.id}")).to eq true
      end
    end
    ...
    context 'if args is Array' do
      ...
      it 'data should not be stored in cache after find' do
        ...
        expect(ActiveCache.exist?("active_cache/user/find/#{user.id}")).to eq false
        expect(ActiveCache.exist?("active_cache/user/find/#{user2.id}")).to eq false
      end
    end
  end
end
